...
router.get('/getLocations', VerifyToken, (req, res, next) => {
  ...
  var queryString =
    'SELECT (row_to_json(ciudad.*)) \
                     FROM (SELECT ciudades.id, ciudades.nombre, ciudades.esquema, \
                      ciudades.latitud, ciudades.longitud, ciudades.zoom FROM public.ciudades \
...
router.get('/getLocationsLayers', (req, res, next) => {
...
router.get('/getTreats', VerifyToken, (req, res, next) => {
  ...
  var queryString =
    'SELECT row_to_json(amenazas.*) FROM \
