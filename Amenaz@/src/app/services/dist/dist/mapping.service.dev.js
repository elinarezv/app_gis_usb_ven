"use strict"; /// <reference path="../leaflet.pattern.d.ts"/>
...
function () {
  function MappingService(authService, storage, env, androidPermissions, geolocation, locationAccuracy, http, events) {
    ...
    this.baseMapName = 'esri-dark'; // Fix Leaflet bug with markers
    ...
    this.loadLocations().subscribe(function (result) {
      ...
      _this.cities.forEach(function (city) {
        city.marker = leaflet_1.marker(city.location, {
          icon: iconVar
  ...
  MappingService.prototype.downloadLocations = function () {
    ...
    return rxjs_1.from(this.storage.getItem('cities')).pipe(operators_1.catchError(function (error) {
      return _this.loadLocations();
    }));
  };
  ...
  MappingService.prototype.getGeoJSON = function (schemaName, layerName) {
    ...
    return this.http.get(this.env.API_URL + 'data/getlayer', {
      headers: {
        Authorization: 'Basic ' + this.authService.token,
        'x-access-token': this.authService.token.data.token
      },
  ...
  MappingService.prototype.getLayer = function (schemaName, layerName) {
    ...
    return rxjs_1.from(this.storage.getItem(layerName)).pipe(operators_1.catchError(function (error) {
      return _this.getGeoJSON(schemaName, layerName);
    }));
  };
  ...
  MappingService.prototype.downloadLocationsLayers = function (cityID) {
    ...
    return rxjs_1.from(this.storage.getItem('ciyLtayers-id' + String(cityID))).pipe(operators_1.catchError(function (error) {
      return _this.loadLocationsLayers(cityID);
    }));
  };
            ...
            onEachFeature: function onEachFeature(feature, layer) {
              ...
              if (feature.properties) {
                ...
                if (feature.properties.Espesor) {
                  layer.setStyle({
                    weight: feature.properties.Espesor
                  });
                }
                ...
                if (feature.properties.ColorRelleno) {
                  layer.setStyle({
                    fillColor: feature.properties.ColorRelleno,
                    fill: true
                  });
                }
                ...
                if (feature.properties.Opacidad) {
                  layer.setStyle({
                    fillOpacity: feature.properties.Opacidad
                  });
                }
  ...
  MappingService = __decorate([core_1.Injectable({
    providedIn: 'root'
  })], MappingService);
